...
import {
  ...
  Clock, ShieldCheck, MessageCircle, DollarSign, ExternalLink,
} from 'lucide-react';
...
export default function PhysiotherapistDetail() {
  ...
  const openInGoogleMaps = (latitude, longitude, clinicName) => {
    ...
    window.open(`https://www.google.com/maps?q=${lat},${lng}&label=${label}`, '_blank', 'noopener,noreferrer');
  };

  // Flow gratis dari halaman ini: tidak ada screeningId,
  // jadi hanya arahkan ke /physiotherapists untuk pilih lewat screening
    ...
    <div className="physio-detail-page">
      <div className="physio-detail-container">
        <div className="physio-detail-empty">
          ...
          <button onClick={() => navigate('/physiotherapists')} className="physio-detail-empty-btn">
        ...
        <div className="physio-detail-card">
          ...
          <div className="physio-detail-header">
            <div className="physio-detail-header-content">
              {physio.photo_url ? (
                <img src={physio.photo_url} alt={physio.name} className="physio-detail-avatar" />
              ) : (
          ...
          <div className="physio-detail-body">
            {(physio.bio_short || physio.bio) && (
              <div className="physio-detail-section">
                ...
                <p className="physio-detail-section-text">{physio.bio_short || physio.bio}</p>
              </div>
            )}
              ...
              <div className="physio-detail-info-grid">
                <div className="physio-detail-info-item">
                  ...
                  <div className="physio-detail-info-content">
                    ...
                    <span className="physio-detail-info-value">{physio.clinic_name || 'Praktik fisioterapi'}</span>
                  </div>
                </div>
                ...
                <div className="physio-detail-info-item physio-detail-info-item--location">
                  ...
                  <div className="physio-detail-info-content">
                    ...
                    <span className="physio-detail-info-value">{physio.city || 'Tidak tersedia'}</span>
                    {physio.latitude && physio.longitude && (
                      <button onClick={() => openInGoogleMaps(physio.latitude, physio.longitude, physio.clinic_name)} className="physio-detail-map-link">
            ...
            {/* ✅ CTA — aktif, buka ConsultationModal */}
            <div className="physio-detail-cta">
              {isLoggedIn ? (
                <button
                  onClick={() => setConsultModal(true)}
                  className="physio-detail-cta-btn"
                >
                  <MessageCircle size={18} strokeWidth={2} />
                  <span>Konsultasi dengan {physio.name?.split(' ')[0]}</span>
                </button>
              ) : (
                <button
                ...
                >
                  <MessageCircle size={18} strokeWidth={2} />
                  <span>Masuk untuk Konsultasi</span>
                </button>
              )}
              <p className="physio-detail-cta-note">
                Pilih antara chat langsung (premium) atau rujukan gratis
              </p>
            </div>
          </div>
        </div>
      ...
      <footer className="physio-detail-footer">
        <div className="physio-detail-footer__inner">
          <div className="physio-detail-footer__brand">
            ...
            <p>Platform screening postur anak berbasis AI yang membantu orang tua berkolaborasi dengan fisioterapis untuk tumbuh kembang yang lebih sehat.</p>
          </div>
