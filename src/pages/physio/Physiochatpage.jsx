...
function MessageBubble({ msg, isMe }) {
  ...
  return (
    <div className={`relative max-w-[85%] sm:max-w-[75%] px-4 py-2.5 text-[15px] leading-relaxed shadow-sm ${
      isMe 
      ...
      {type === "video" && fileUrl && (
        <video
          src={fileUrl}
          controls
          className={`max-w-full max-h-52 rounded-lg block bg-black ${text ? 'mb-2' : ''}`}
        />
      )}
      {type === "file" && fileUrl && (
        <a
          ...
          className={`flex items-center gap-2.5 p-2.5 rounded-lg no-underline w-full max-w-sm transition-colors active:scale-95 ${
        ...
        >
          ...
          <span className="text-xs font-semibold overflow-hidden text-ellipsis whitespace-nowrap flex-1">
            {msg.file_name || "File Dokumen"}
          </span>
          <Download size={14} className="shrink-0 opacity-70" />
        </a>
      )}
      {text && <span className="block break-words">{text}</span>}
    </div>
  );
}
...
function FilePreview({ file, onRemove }) {
  ...
  return (
    <div className={`relative inline-flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg max-w-[240px] shadow-sm mb-2 ${isImage ? 'p-1.5' : 'py-2 px-3'}`}>
      ...
      <button onClick={onRemove} className="absolute -top-2 -right-2 bg-red-500 border-2 border-white rounded-full w-5 h-5 flex items-center justify-center cursor-pointer text-white shadow-sm hover:bg-red-600 active:scale-95 transition-all">
...
export default function PhysioChatPage() {
  ...
  // Global UI States (Konsisten dengan Dashboard)
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      ...
      >
        ...
        <nav className="flex-1 px-3 space-y-1 mt-4 hide-scrollbar overflow-y-auto overflow-x-hidden">
          ...
          {/* Label diganti menjadi Konsultasi */}
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full bg-white overflow-hidden">
        ...
        <header className="h-16 lg:h-20 bg-white border-b border-slate-200 flex items-center justify-between px-4 lg:px-6 shrink-0 z-10">
          ...
          <div className="flex items-center gap-2 md:gap-4 ml-auto lg:ml-0">
            {/* Daftar Premium Badge Highlight */}
        ...
        <div className="flex-1 flex overflow-hidden bg-white">
              ...
              {loadingConvs ? (
                ...
                <div className="divide-y divide-slate-100">
                  {conversations.map((conv) => {
                    ...
                    // Tema list jika premium vs reguler
                    const baseClass = isActive ? "bg-blue-50" : "bg-white hover:bg-slate-50";
                    const premiumBorder = isPremium && !isActive ? "border-l-4 border-l-amber-400" : isActive ? "border-l-4 border-l-blue-600" : "border-l-4 border-l-transparent";
                    ...
                    return (
                      ...
                      >
                        <div className={`w-11 h-11 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0 shadow-sm ${isPremium ? 'bg-amber-500' : 'bg-gradient-to-br from-blue-500 to-indigo-500'}`}>
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-center">
                          ...
                          <div className="flex justify-between items-center gap-2">
                            ...
                            {conv.unread_count > 0 && (
                              <div className="w-2.5 h-2.5 bg-blue-600 rounded-full shrink-0 shadow-sm" />
                            )}
                          </div>
                        </div>
                      </div>
          ...
          <div className={`${!isMobileChatActive ? 'hidden md:flex' : 'flex'} flex-1 flex-col bg-slate-50/50 min-w-0 relative`}>
            {activeConv ? (
              <>
                {/* Header Active Chat */}
                <div className="h-16 px-4 bg-white border-b border-slate-200 flex items-center gap-3 shrink-0 sticky top-0 z-10 shadow-sm">
                  <button className="md:hidden p-1.5 -ml-2 text-slate-500 hover:bg-slate-100 rounded-md active:scale-95 transition-all" onClick={handleCloseConversation}>
                    <ChevronLeft size={24} />
                  </button>
                          ...
                          <div className={`flex w-full mt-2 ${isMe ? "justify-end" : "justify-start"} ${msg._optimistic ? "opacity-60" : "opacity-100"}`}>
                            <div className={`flex flex-col ${isMe ? "items-end" : "items-start"} max-w-full`}>
                              ...
                              <div className="flex items-center gap-1.5 mt-1 mx-1">
                                ...
                                {msg._optimistic && <span className="text-[10px] font-medium text-blue-500 italic">· Mengirim...</span>}
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                {/* Input Area */}
                <div className="p-3 sm:p-4 bg-white border-t border-slate-200 shrink-0 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)]">
                  ...
                  {/* Button Presisi Center Height 44px */}
                  <div className="flex items-end gap-2 sm:gap-3">
                    <button onClick={() => fileInputRef.current?.click()} disabled={sending} className="w-[44px] h-[44px] flex items-center justify-center text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all active:scale-95 disabled:opacity-50 shrink-0">
                      <Paperclip size={20} />
                    </button>
                    <button onClick={() => cameraInputRef.current?.click()} disabled={sending} className="w-[44px] h-[44px] flex items-center justify-center text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all active:scale-95 disabled:opacity-50 shrink-0 hidden sm:flex">
                      <Camera size={20} />
                    </button>
                    ...
                    <button 
                      className={`w-[44px] h-[44px] rounded-xl flex items-center justify-center transition-all active:scale-95 shrink-0 shadow-sm ${(!inputText.trim() && !selectedFile) || sending ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
                      onClick={handleSend} 
...
// Skeleton untuk Sidebar (Daftar Pesan)
...
// Skeleton untuk Ruang Chat (Isi Pesan)
function ChatMessagesSkeleton() {
  return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="flex w-full justify-start">
        <div className="flex flex-col items-start gap-1.5">
          <div className="h-20 w-56 bg-slate-200 rounded-2xl rounded-tl-sm"></div>
          <div className="h-2.5 w-12 bg-slate-200 rounded ml-1"></div>
        </div>
      </div>
    </div>
