// src/pages/ScreeningDetailPage.jsx
import { useEffect, useState } from "react";
import { useParams, useNavigate } from "react-router-dom";
import {
  Calendar, AlertCircle, User, Camera, FileText,
  ChevronLeft, ChevronRight, X, Sparkles,
  Plus, History, AlertTriangle, MessageCircle, Download,
} from "lucide-react";
...
function ScreeningDetailPage() {
  ...
  // ── Loading / Error / Empty ───────────────────────────────────────────
  if (loading) return (
    <div className="sd-page"><div className="sd-container"><div className="sd-loading">Memuat data screening...</div></div></div>
  );
  if (error) return (
    <div className="sd-page"><div className="sd-container">
      <div className="sd-error">
        ...
        <p>{error}</p>
        <button onClick={() => navigate(-1)} className="sd-btn sd-btn--secondary">Kembali</button>
      </div>
    </div></div>
  );
  if (!data) return (
    <div className="sd-page"><div className="sd-container"><p className="sd-no-data">Data tidak ditemukan.</p></div></div>
  );
    ...
    <div className="sd-page">
      <div className="sd-container">
        ...
        <div className="sd-top-actions">
          <button onClick={() => navigate(`/children/${child?.id}/screenings/new`)} className="sd-top-btn sd-top-btn--primary">
            <Plus size={16} strokeWidth={2} /> Screening Baru
          </button>
          <button onClick={() => navigate(`/children/${child?.id}/screenings`)} className="sd-top-btn sd-top-btn--secondary">
            <History size={16} strokeWidth={2} /> Riwayat
          </button>
          {/* ✅ Export PDF */}
          <button onClick={() => exportScreeningPDF(data)} className="sd-top-btn sd-top-btn--secondary" title="Export PDF">
            <Download size={16} strokeWidth={2} /> Export PDF
          </button>
        </div>
        ...
        <section className="sd-info-score-row">
          <div className="sd-info-col">
            ...
            <div className="sd-info-meta">
              ...
              <span>{child?.gender === "M" ? "Laki-laki" : child?.gender === "F" ? "Perempuan" : "-"}</span>
            </div>
            <div className="sd-info-meta">
            ...
            <div className="sd-date-row">
              ...
              {is_multi_view && (<><span className="sd-separator">·</span><span className="sd-multi-label">{total_views} foto</span></>)}
            </div>
          </div>
          <div className="sd-score-col">
            <div className="sd-circle" style={{ borderColor: currentCat.color }}>
              <span style={{ color: currentCat.color }}>{score != null ? Number(score).toFixed(1) : "-"}</span>
            </div>
            <span className="sd-badge" style={{ backgroundColor: currentCat.color }}>{currentCat.label}</span>
          </div>
        </section>
          ...
          <div className="sd-left-panel">
            {metrics && Object.keys(metrics).length > 0 && (
              <section className="sd-metrics-table">
                <div className="sd-metrics-header">
                  ...
                  {hasAnyDeviation && (
                    <button className="sd-deviation-cta" onClick={() => setShowDeviationModal(true)}>
                      <AlertTriangle size={14} strokeWidth={2} /> Lihat Area Deviasi
                    </button>
                  )}
                </div>
                <table>
                  ...
                  <tbody>
                    {metrics.shoulder_tilt_index !== undefined && (
                      <tr><td>Kemiringan Bahu</td><td>{metrics.shoulder_tilt_index.toFixed(1)}%</td>
                        <td><span className={metrics.shoulder_tilt_index < 2 ? "status-ok" : "status-warn"}>{metrics.shoulder_tilt_index < 2 ? "Normal" : "Deviasi"}</span></td></tr>
                    )}
                    {metrics.hip_tilt_index !== undefined && (
                      <tr><td>Kemiringan Panggul</td><td>{metrics.hip_tilt_index.toFixed(1)}%</td>
                        <td><span className={metrics.hip_tilt_index < 2 ? "status-ok" : "status-warn"}>{metrics.hip_tilt_index < 2 ? "Normal" : "Deviasi"}</span></td></tr>
                    )}
                    {metrics.forward_head_index !== undefined && (
                      <tr><td>Forward Head Posture</td><td>{metrics.forward_head_index.toFixed(2)}</td>
                        <td><span className={metrics.forward_head_index < 0.2 ? "status-ok" : "status-warn"}>{metrics.forward_head_index < 0.2 ? "Normal" : "Forward Head"}</span></td></tr>
                    )}
                    {metrics.neck_inclination_deg !== undefined && (
                      <tr><td>Kemiringan Leher</td><td>{metrics.neck_inclination_deg.toFixed(1)}°</td>
                        <td><span className={metrics.neck_inclination_deg < 15 ? "status-ok" : "status-warn"}>{metrics.neck_inclination_deg < 15 ? "Normal" : "Deviasi"}</span></td></tr>
                    )}
                    {metrics.torso_inclination_deg !== undefined && (
                      <tr><td>Kemiringan Punggung</td><td>{metrics.torso_inclination_deg.toFixed(1)}°</td>
                        <td><span className={metrics.torso_inclination_deg < 15 ? "status-ok" : "status-warn"}>{metrics.torso_inclination_deg < 15 ? "Normal" : "Deviasi"}</span></td></tr>
                    )}
                  </tbody>
                </table>
              </section>
            ...
            {category !== "GOOD" && (
              <section className="sd-referral-card">
                <h3><User size={16} strokeWidth={1.5} /> Status Konsultasi Fisioterapis</h3>
                ...
                {canChat && (
                  <button
                    onClick={() => handleOpenChat(physiotherapist)}
                    className="sd-cta sd-cta--chat"
                    style={{
                      display: "flex", alignItems: "center", gap: "0.5rem",
                      marginTop: canRefer ? "0.5rem" : 0,
                      background: "#4f7aff", color: "white", border: "none",
                      borderRadius: "10px", padding: "0.625rem 1.125rem",
                      fontWeight: 600, fontSize: "0.875rem", cursor: "pointer",
                    }}
                  >
                    <MessageCircle size={15} strokeWidth={2} />
                    Chat dengan {physiotherapist.name}
                  </button>
                )}
          ...
          <div className="sd-right-panel">
            {mainImages.length > 0 && (
              <section className="sd-image-card">
                <div className="sd-image-card-header">
                  <h3><Camera size={16} strokeWidth={1.5} /> Foto Analisis Postur</h3>
                </div>
                {mainImages.length > 1 && (
                ...
                {currentImage && (
                  <div className="sd-image-wrapper">
                    <img src={currentImage.url_processed || currentImage.url_original} alt={currentImage.type} className="sd-image" />
                  </div>
                )}
                {mainImages.length > 1 && (
                  <div className="sd-image-controls">
                    <button onClick={prevImage} className="sd-nav-btn"><ChevronLeft size={16} strokeWidth={2} /></button>
                    <span className="sd-image-count">{selectedImageIndex + 1} / {mainImages.length}</span>
                    <button onClick={nextImage} className="sd-nav-btn"><ChevronRight size={16} strokeWidth={2} /></button>
                  </div>
                )}
              </section>
            )}
            {hasAIRec && (
              <button className="sd-cta sd-cta--primary-fill" onClick={() => setShowAIModal(true)}>
                <Sparkles size={16} strokeWidth={2} /> Lihat Rekomendasi AI
              </button>
            )}
          </div>
        </div>
        ...
        <section id="sd-fisio-section" className="sd-manual-rec">
          <h3><FileText size={18} strokeWidth={1.5} /> Rekomendasi dari Fisioterapis</h3>
            ...
            <ul className="sd-manual-list">
              {manualRecommendations.map((rec) => (
                <li key={rec.id}>
                  ...
                  <p>{rec.content}</p>
                  <small>oleh {rec.physio?.name || "Fisioterapis"} pada {rec.created_at ? new Date(rec.created_at).toLocaleDateString("id-ID") : "-"}</small>
                </li>
              ))}
            </ul>
          ) : (
        ...
        <div className="sd-modal-overlay" onClick={() => setShowAIModal(false)}>
          <div className="sd-modal" onClick={(e) => e.stopPropagation()}>
            <div className="sd-modal__header">
              ...
              <button onClick={() => setShowAIModal(false)} className="sd-modal__close"><X size={20} strokeWidth={2} /></button>
            </div>
            <div className="sd-modal__body">
              {currentImage.recommendations.map((rec, idx) => (
                <div key={idx} className="sd-rec-item">
                  ...
                  <div className="sd-rec-exercise"><strong>Latihan</strong><p>{rec.exercise}</p></div>
                  <p className="sd-rec-duration"><strong>Durasi:</strong> {rec.duration}</p>
                  {rec.parent_note && <div className="sd-rec-note">{rec.parent_note}</div>}
                  {rec.video_url && <a href={rec.video_url} target="_blank" rel="noopener noreferrer" className="sd-rec-video-link">Lihat Video Tutorial</a>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="sd-modal-overlay" onClick={() => setShowDeviationModal(false)}>
          <div className="sd-modal sd-modal--small" onClick={(e) => e.stopPropagation()}>
            <div className="sd-modal__header">
              ...
              <button onClick={() => setShowDeviationModal(false)} className="sd-modal__close"><X size={20} strokeWidth={2} /></button>
            </div>
            <div className="sd-modal__body">
              {hasAnyDeviation ? (
                <div className="sd-deviation-list">
                  {shoulderDeviated && <DeviationRow label="Kemiringan Bahu"     metric={metrics.shoulder_tilt_index.toFixed(1)   + "%"} note="Bahu kiri-kanan tidak sejajar." />}
                  {hipDeviated      && <DeviationRow label="Kemiringan Panggul"  metric={metrics.hip_tilt_index.toFixed(1)        + "%"} note="Panggul kiri-kanan tidak sejajar." />}
                  {headDeviated     && <DeviationRow label="Forward Head Posture" metric={metrics.forward_head_index.toFixed(2)}        note="Kepala terlalu maju ke depan." />}
                  {neckDeviated     && <DeviationRow label="Kemiringan Leher"    metric={metrics.neck_inclination_deg.toFixed(1)  + "°"} note="Leher cenderung menunduk." />}
                  {torsoDeviated    && <DeviationRow label="Kemiringan Punggung" metric={metrics.torso_inclination_deg.toFixed(1) + "°"} note="Punggung cenderung membungkuk." />}
                  {cropImages.length > 0 && (
                    <div className="sd-deviation-crops">
                      ...
                      <div className="sd-deviation-crops-grid">
                        {cropImages.map((crop) => {
                          ...
                          const displayName = { SHOULDER: "Bahu", HIP: "Panggul", HEAD: "Kepala", NECK: "Leher", TORSO: "Punggung" }[regionName] || regionName;
                          return (
                            <div key={crop.id} className="sd-deviation-thumb">
                              <img src={crop.url_original} alt={`Crop ${regionName}`} />
      ...
      {/* ── Modal Pilih Fisioterapis (gratis) ── */}
      {isReferModalOpen && (
        <div className="sd-modal-overlay" onClick={() => setIsReferModalOpen(false)}>
          <div className="sd-modal" onClick={(e) => e.stopPropagation()}>
            <div className="sd-modal__header">
              <h3>Pilih Fisioterapis</h3>
              <button onClick={() => setIsReferModalOpen(false)} className="sd-modal__close"><X size={20} strokeWidth={2} /></button>
            </div>
            <div className="sd-modal__body">
              {loadingPhysios ? (
      ...
      {/* ── Modal Konfirmasi Rujukan Gratis ── */}
        ...
        <div className="sd-modal-overlay" onClick={() => setConfirmReferModal({ show: false, physioId: null, physioName: "" })}>
          <div className="sd-modal sd-modal--small" onClick={(e) => e.stopPropagation()}>
            <div className="sd-modal__header">
              ...
              <button onClick={() => setConfirmReferModal({ show: false, physioId: null, physioName: "" })} className="sd-modal__close"><X size={20} strokeWidth={2} /></button>
            </div>
            <div className="sd-modal__body">
              <p style={{ marginBottom: "1.5rem", lineHeight: "1.6" }}>
                Apakah Anda yakin ingin mengirimkan hasil screening ini ke <strong>{confirmReferModal.physioName}</strong> untuk konsultasi lebih lanjut?
              </p>
              <div style={{ display: "flex", gap: "0.75rem" }}>
                <button onClick={() => setConfirmReferModal({ show: false, physioId: null, physioName: "" })} className="sd-btn sd-btn--secondary" style={{ flex: 1 }}>Batal</button>
                <button onClick={() => handleRefer(confirmReferModal.physioId)} disabled={submittingRefer} className="sd-btn sd-btn--primary" style={{ flex: 1 }}>
